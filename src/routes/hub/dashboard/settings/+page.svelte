<script lang="ts">
  ...
  const createAccountLink = async () => {
    ...
    const stripeAccount = await getDoc(
      doc(db, "stripe-accounts", $authStore.currentUser["uid"])
    );

    if (stripeAccount.exists()) {
      try {
      ...
      try {
        let stripeAccount: any = await axios.post("/api/createStripeAccount");

        await setDoc(
          doc(db, "stripe-accounts", $authStore.currentUser["uid"]),
          {
            stripeAccountId: stripeAccount.data.id,
          }
        );

        let accountLink: any = await axios.post("/api/createAccountLink", {
          id: stripeAccount.data.id,
        });
        window.location.href = accountLink.data.url;
      } catch (error) {
        console.log(error);
  ...
  const checkStripeAccount = async () => {
    ...
    if (!stripeAccountId.exists()) return;
...
<div
  class="w-full h-full rounded border border-neutral-500 p-5 overflow-scroll"
>
  {#if !loading}
